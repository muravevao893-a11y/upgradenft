...
- Добавлена авто-локализация по приоритету: сохраненный язык -> `Telegram user.language_code` -> `navigator.language`.
- Поддерживаются языки: `RU`, `EN`, `UK`.
- В профиле есть язык-пикер с поиском по языку/стране и SVG-флагами.
- Выбранный язык сохраняется в `localStorage` (`upnft_locale_v1`).

## Профиль устройства

- Добавлено автоопределение профиля устройства (`mobile` / `desktop`) с учетом платформы Telegram и viewport.
- В desktop-профиле включается расширенная раскладка (двухколоночные апгрейды, более широкие сетки), а в узком окне desktop возвращается компактный режим.
